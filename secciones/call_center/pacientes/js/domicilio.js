    ...
    if (regex.test(numero)) {
        if(numero.length==5){
            ...
            .then(data => {
                ...
                data.forEach(dato => {
                    console.log(dato);
                    colonia.innerHTML+="<option value='"+dato.nombre+"'>"+dato.nombre+"</option>";
                    delMun.innerHTML="<option value='"+dato.municipioName+"'>"+dato.municipioName+"</option>";
                    estadoDir.innerHTML="<option value='"+dato.estadoName+"'>"+dato.estadoName+"</option>";
                });
            })
            .catch(error => console.error("Error: "+error));
        } else {
            colonia.innerHTML='<option value="">Selecciona un Código Postal</option>';
            delMun.innerHTML='<option value="">Selecciona un Código Postal</option>';
            estadoDir.innerHTML='<option value="">Selecciona un Código Postal</option>';
        }
    } else {
