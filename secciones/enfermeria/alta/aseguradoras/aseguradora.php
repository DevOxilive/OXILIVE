...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->MultiCell(260, 5, utf8_decode('Listado de Aseguradoras'), 0, 'C');
        // Salto de línea
        $this->Ln(25);
    }
    // Pie de página
...
$sentencia = $con->prepare("SELECT *,(SELECT Nombre_administradora FROM administradora WHERE administradora.id_administradora = aseguradoras.administradora) AS administradora FROM aseguradoras");
$sentencia->execute();
$pdf->Ln(0.6);
$pdf->setX(15);
$pdf->SetFont('Times', 'B', 12);
$pdf->Cell(60, 10, 'Nombre de la Aseguradora',1, 0, 'C', 0);
$pdf->Cell(60, 10, 'Administradora',1, 0, 'C', 0);
$pdf->Cell(40, 10, 'Fecha de registro', 1, 1, 'C', 0);
while ($userRow = $sentencia->fetch(PDO::FETCH_ASSOC)) {
    $pdf->SetFont('Arial', '', 12);
    $pdf->setX(15);
    $pdf->Cell(60, 10, utf8_decode($userRow['Nombre_aseguradora']), 1, 0, 'C', 0);
    $pdf->Cell(60, 10, utf8_decode($userRow['administradora']), 1, 0, 'C', 0);
    $pdf->Cell(40, 10, $userRow['Fecha_registro'], 1, 1, 'C', 0);
