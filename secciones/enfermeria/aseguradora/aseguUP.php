...
if (isset($_GET['txtID'])) {
  ...
  $sentencia = $con->prepare("SELECT * FROM asegu_enfer WHERE id_asegu_enfer=:id_asegu_enfer");
  $sentencia->bindParam(":id_asegu_enfer", $txtID);
  $sentencia->execute();
...
if ($_POST) {
  ...
  if ($resul > 0) {
    ...
    $sentencia = $con->prepare("UPDATE asegu_enfer 
    SET Nombre_aseguradora=:Nombre_aseguradora, admi=:admi
                WHERE id_asegu_enfer=:id_asegu_enfer");
