    ...
    fetch("../model/status.php", {
      ...
      .then((datos) => {
        datos.forEach((dato) => {
          ...
          if(dato.id_est === 1){
            btns.innerHTML = "<a class='btn btn-outline-info' href='pages/editar.php?idHor=" + dato.id + "' role='button'><i class='i bi-pencil-square'></i></a> | <a class='btn btn-outline-danger' role='button' href='#' onclick='cancelHor(event, "+ dato.id +")'><i class='bi bi-x-lg'></i></a>"
          }
