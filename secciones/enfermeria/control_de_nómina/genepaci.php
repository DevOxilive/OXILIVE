...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->MultiCell(260, 5, utf8_decode('Pacientes activos'), 0, 'C');
        // Salto de línea
        $this->Ln(25);
    }
    // Pie de página
...
$sentencia = $con->prepare("SELECT *,(SELECT Nombre_administradora FROM administradora WHERE administradora.id_administradora = pacientes_oxigeno.Administradora LIMIT 1)as administradora FROM pacientes_oxigeno WHERE estado != 5");
$sentencia->execute();

$pdf->Ln(0.6);
$pdf->setX(15);
$pdf->SetFont('Times', 'B', 14);
$pdf->Cell(75, 10, 'Nombre del Paciente',1, 0, 'C', 0);
$pdf->Cell(40, 10, 'Administradora',1, 0, 'C', 0);
...
while ($userRow = $sentencia->fetch(PDO::FETCH_ASSOC)) {
    ...
    $pdf->Cell(40, 10, utf8_decode($userRow['administradora']) , 1, 0, 'C', 0);
    $pdf->Cell(35, 10, $userRow['No_nomina'], 1, 1, 'C', 0);
}
$pdf->Output();
?>