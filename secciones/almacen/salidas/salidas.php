...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->MultiCell(260, 5, utf8_decode('Salidas de Almacén'), 0, 'C');
        // Salto de línea
        $this->Ln(25);
    }
    // Pie de página
...
$sentencia = $con->prepare("SELECT *,(SELECT Nombres FROM empleados WHERE empleados.id_empleados=salida_almacen.pide_salida LIMIT 1) as saco FROM salida_almacen");
$sentencia->execute();
$pdf->Ln(0.6);
$pdf->setX(15);
$pdf->SetFont('Times', 'B', 12);
$pdf->Cell(45, 10, 'Nombre del producto',1, 0, 'C', 0);
$pdf->Cell(35, 10, 'Cantidad sacada',1, 0, 'C', 0);
$pdf->Cell(55, 10, 'Persona a quien se le entrego',1, 0, 'C', 0);
$pdf->Cell(55, 10, 'Fecha de salida', 1, 1, 'C', 0);
while ($userRow = $sentencia->fetch(PDO::FETCH_ASSOC)) {
    $pdf->SetFont('Arial', '', 11);
    $pdf->setX(15);
    $pdf->Cell(45, 10, utf8_decode($userRow['nombre_matesali']), 1, 0, 'C', 0);
    $pdf->Cell(35, 10, utf8_decode($userRow['cantidad_salida']), 1, 0, 'C', 0);
    $pdf->Cell(55, 10, utf8_decode($userRow['saco']), 1, 0, 'C', 0);
    $pdf->Cell(55, 10, $userRow['fecha_salida'], 1, 1, 'C', 0);
